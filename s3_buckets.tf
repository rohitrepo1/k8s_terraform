...
resource "aws_s3_bucket_object" "ark" {
    bucket = "${aws_s3_bucket.script_s3_bucket.id}"
    acl    = "public-read"
    key = "arksetup.sh"
    source = "./scripts/arksetup.sh"
    etag = "${md5(file("./scripts/arksetup.sh"))}"
}
...
resource "aws_s3_bucket" "heptioark_s3_bucket" {
    bucket        = "heptioark.${var.client}.devops"
    acl           = "private"
    force_destroy = true
